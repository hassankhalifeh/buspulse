...
// Private phone-login link of one fleet, with a QR code that is drawn in the browser
// (the link is never sent to a third-party QR service).
export default function FleetLoginLink({ slug, fleetName }: { slug: string; fleetName: string }) {
  const [url, setUrl] = useState("");
  const [qr, setQr] = useState("");
  const [copied, setCopied] = useState(false);

  useEffect(() => {
    const u = `${window.location.origin}/phone-login?f=${slug}`;
    setUrl(u);
  ...
  return (
    <div className="card" style={{ padding: "1.25rem", marginBottom: 14 }}>
      ...
      <p style={{ fontSize: "0.82rem", color: "var(--steel)", margin: "0 0 12px" }}>
        وزّع هذا الرابط أو رمز QR الخاص بأسطولك فقط. لا يعمل الدخول بالرقم إلا من خلاله.
      </p>
      <div style={{ display: "flex", gap: 16, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <div style={{ flex: 1, minWidth: 200 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            {qr && (
              <a href={qr} download={`login-qr-${slug}.png`} className="btn btn-secondary" style={{ fontSize: "0.85rem", padding: "6px 12px" }}>
